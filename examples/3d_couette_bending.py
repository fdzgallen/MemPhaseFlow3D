...
simulation="Couette_1" #Simulation = Output directory of data
print(simulation)
...
#Solving Poisson equation with given function b of vector p, using MASK geometry. Returns the solution vector of the equation
...
def periodic_gradN(u): #gradient of a given field U with Neumann Boundary Conditions
    gu = [-(np.roll(u,1,0)-np.roll(u,-1,0))*0.5/h, -(np.roll(u,1,1)-np.roll(u,-1,1))*0.5/h]
    gu[1][:,0]=0.0
    gu[1][:,-1]=0.0
    return gu
...
def plot_all(phi,w,curr,t):
    ...
    axarr[1,0].quiver(X[::3,::3],Y[::3,::3],periodic_gradN(curr)[1][::3,::3].T,-periodic_gradN(curr)[0][::3,::3].T);
    ...
    vmoduleA=vmodule()
    absol = np.absolute(vmoduleA)
    minmax=np.amax(absol)
#    print((curr-currin).sum())
    
    currplot2=axarr[1,1].imshow((vmoduleA).T,cmap='Greens',origin='lower'
    #, vmin = -minmax, vmax = minmax
    )
    f.colorbar(currplot2, ax=axarr[1,1]
   # , ticks=[-minmax*2/3,0,minmax*2/3]
    ,fraction=0.035  , orientation='horizontal',format=FormatStrFormatter('%.3f'))
    axarr[1,1].autoscale(False)
    if ( (vmoduleA).sum()*(vmoduleA).sum() > 0.000000):
        axarr[1,1].quiver(X[::5,::5],Y[::5,::5],periodic_gradN(A_i0mod)[1][::5,::5].T,-periodic_gradN(A_i0mod)[0][::5,::5].T);
        

    axarr[0,1].set_title('Vorticity')
    axarr[1,0].set_title('Stream function ξ')
    axarr[1,1].set_title(r'$\sqrt{v_x^2+v_y^2}$')

    figure = plt.gcf()
    figure.set_size_inches(10, 9.0,forward=True)
    plt.tight_layout()
...
def dump(t,start,curr,currin,w,vmoduleA):
...
############################################################################### PHI
        f, axarr = plt.subplots(2,2)
        phiplot=axarr[0,0].imshow(phi[:,int(Ny*0.5),:].T,cmap='seismic',origin='lower')
        f.colorbar(phiplot, ax=axarr[0,0], ticks=[-0.95,0,0.95],fraction=0.035, orientation='horizontal')
...
#Creating boolean matrix newbooll. It indicates the points where you calculate evolutions. Newbooll - booll = boundary points
newbooll2 = booll2.copy()
newbooll2[:,:,-1] = 0; newbooll2[:,:,0] = 0; 
    ...
    for j in range(0,Ny):
        if(booll[i][j]): #just if you ara inside the cylinder
            for k in range(0,Nz):
                ...
                w_x[i,j,k] = 0
                w_z[i,j,k] = 0
                A_y[i,j,k] = -vwall*float(k**2)/(2*H)
                A_x[i,j,k] = 0
                v_x[i,j,k] = vwall*float(k/H)
...
for t in range(0,t1):
    ...
    if (t % tdump == 0): 
        ...
        plotv = (v_x*v_x + v_z*v_z)**(0.5)
        dump(t,start,A_mod[:,int(Ny*0.5),:],A_i0mod[:,int(Ny*0.5),:],w_mod[:,int(Ny*0.5),:],plotv[:,int(Ny*0.5),:])
...
for t in range(t1,Tend):
    ...
    v_x[maskT] = grad_Az[1][maskT] - grad_Ay[2][maskT]
    v_y[maskT] = grad_Ax[2][maskT] - grad_Az[0][maskT]
    v_z[maskT] = grad_Ay[0][maskT] - grad_Ax[1][maskT]  
