...
#
# This model simulates a surface tension mediated deformable object like an oil droplet
# simulated in a Poiseuille flow in a cylindrical tube under a fluid flow in 3D.
...
simulation="output/CahnHilliard_descentered_30" #Simulation = Output directory of data
print(simulation)
...
alpha2= 0.0 # Strength of the lagrange mutiplier effect for AREA
...
phi = -unity
w_x=np.zeros((Nx,Ny,Nz),float); w_y=np.zeros((Nx,Ny,Nz),float); w_z=np.zeros((Nx,Ny,Nz),float)
w_mod = np.zeros((Nx,Ny,Nz),float)
v_x=np.zeros((Nx,Ny,Nz),float); v_y=np.zeros((Nx,Ny,Nz),float); v_z=np.zeros((Nx,Ny,Nz),float)
v_mod = np.zeros((Nx,Ny,Nz),float)
A_x=np.zeros((Nx,Ny,Nz),float); A_y=np.zeros((Nx,Ny,Nz),float); A_z=np.zeros((Nx,Ny,Nz),float)
A_mod = np.zeros((Nx,Ny,Nz),float)
w_x0 = np.zeros((Nx,Ny,Nz),float); w_y0 = np.zeros((Nx,Ny,Nz),float); w_z0 = np.zeros((Nx,Ny,Nz),float)  
A_x0 = np.zeros((Nx,Ny,Nz),float); A_y0 = np.zeros((Nx,Ny,Nz),float); A_z0 = np.zeros((Nx,Ny,Nz),float) 
...
for t in range(0,t1):
    ...
    if(t < t0): 
        ...
        phi[maskT] += dt*M*tension*(lap_psi[maskT])
        ...
        phi[maskT] += dt*M*tension*(lap_psi[maskT]+sigmavmatrix[maskT])
...
for t in range(t1,Tend):
    ...
    gradpsi = grad_roll(psi_sc,maskT)
