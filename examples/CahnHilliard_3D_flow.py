...
alpha= 0.0 # Strength of the lagrange mutiplier effect for AREA
...
simulation="ST_descentered_035" #Simulation = Output directory of data
print(simulation)
...
def compute_psi(phi0,lap_phi,mask):
    ...
    psi_sc[mask]=phi0[mask]*(-unity[mask]+phi0[mask]*phi0[mask])-eps*eps*lap_phi[mask]
...
for t in range(t1,Tend):
   ...
   # mu_mutipl,lap_mu_multipl = compute_mu_multipl(phi,lap_phi, Sigma, Sigma2, Sigma3,maskT)
    ...
    w_x = poisson(w_x, (gradpsi[2]*gradphi[1] - gradpsi[1]*gradphi[2])/viscosityliq,maskT) #if contrast is =! 1 replace for viscosity
    w_y = poisson(w_y, (gradpsi[0]*gradphi[2] - gradpsi[2]*gradphi[0])/viscosityliq,maskT)
    w_z = poisson(w_z, (gradpsi[1]*gradphi[0] - gradpsi[0]*gradphi[1])/viscosityliq,maskT)
